const ynsTag = "[YoutubeNonStop for Safari - v0.1]";
...
function tryObserveVideoPlayer(){
    if(document.querySelectorAll(videoPlayerElement).length){
        ...
        log("Observing video player!");
        return true;
    }
    else{
        if(!informedVideo){
            log("Searching for video player...");
            informedVideo = true;
        }
        return false;
    }
...
function tryObserveDialog(){
    if(document.querySelectorAll(dialogElement).length){
        ...
        log("Observing confirm dialog!");
        return true;
    }
    else{
        if(!informedDialog){
            log("Searching for confirm dialog...");
            informedDialog = true;
        }
        return false;
    }
...
function tryClickVideoPlayer(){
    if(document.querySelector(videoPlayerElement).classList.contains("paused-mode") && !videoActed){
        ...
        log(getTimestamp() + " " + document.querySelector("head title").innerHTML + " Detected paused video and clicked it to continue!");
    }
}
...
function tryClickDialog(){
    if(document.querySelector(dialogElement).style.display !== 'none' && !dialogActed){
        ...
        log(getTimestamp() + " " + document.querySelector("head title").innerHTML  + " Confirmed watching in dialog!");
    }
}
